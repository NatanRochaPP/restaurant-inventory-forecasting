...
Output: ``docs/home project_draft_V4.docx``

Every number is read from ``outputs/``, every reference is rendered from
``scripts/references.py``, and every in-text citation is checked against that registry by
``scripts/citations.py``; the build stops on a citation it cannot match. The Section 4.1
diagram is drawn from the ``src/`` tree, and the Section 4.7 screenshots are read from
``outputs/report/screenshots/`` (``scripts/capture_dashboard.py``).
"""
...
from reflection_text import CALIBRATION, REFLECTION  # noqa: E402
...
TARGET = DOCS / "home project_draft_V4.docx"
WORD_COUNT_PREFIX = "Word count: "
...
def count_tests() -> int:
    """Tests pytest collects, slow ones included, so the count quoted cannot go stale."""
    result = subprocess.run([sys.executable, "-m", "pytest", "--collect-only", "-q"],
                            cwd=PROJECT_ROOT, capture_output=True, text=True, check=True)
    return int(re.search(r"(\d+) tests? collected", result.stdout).group(1))
...
def title_page(document: Document, ev: dict) -> None:
    ...
    for _ in range(3):
        document.add_paragraph()
    para(document, "Natan Rocha Paiva", size=14, align=WD_ALIGN_PARAGRAPH.CENTER)
    for _ in range(3):
        document.add_paragraph()
    # Filled in by build() once the whole document exists.
    para(document, WORD_COUNT_PREFIX, size=10, colour=MUTED, align=WD_ALIGN_PARAGRAPH.CENTER)
    page_break(document)
...
def chapter_introduction(document: Document, ev: dict) -> None:
    ...
    para(document,
         'The scale of the resulting loss is documented, though less precisely than '
         'its headline figures suggest. WRAP estimates that the UK hospitality and '
         'food service sector generates 1.1 million tonnes of food waste a year. Of '
         'that, 800,000 tonnes is food that could have been eaten, valued at £3.21 '
         "billion at 2021 prices, and the sector's food waste amounts to 18% of the "
         "food it buys (WRAP 2025). WRAP's sector page puts the cost at an average of"
         ' £10,000 per outlet per year (WRAP n.d.).')
    para(document,
    ...
    para(document,
         'A note on the currency of these figures is warranted, because both are '
         'older than their publication dates suggest. The split between preparation, '
         "plate and spoilage waste comes from WRAP's survey of 2011, published in "
         '2013 (WRAP 2013), and no later breakdown of that kind has been published. '
         'The current tonnage is not a new measurement either. WRAP states that its '
         'estimates for the sector are modelled from changes in the number and types '
         'of sites, on the assumption that food waste per site has remained constant '
         'since 2011, and that no data source currently supports a reliable UK-level '
         'estimate for the sector (WRAP 2025). The figures are therefore used here to'
         ' establish scale and composition, not as a current measurement, and no part'
         ' of the argument depends on their precision. What the argument requires is '
         'only the ordering: that spoilage is a substantial but minority component, '
         'and the only one an ordering decision can reach.')
    para(document,
    ...
    para(document,
         ...
         "concern supermarkets. None of the sources reviewed studies ordering behaviour in small, "
         "independent hospitality operations, and the differences between the settings - shorter "
...
def chapter_methodology(document: Document, ev: dict) -> None:
    ...
    para(document,
         'One qualification belongs with that guarantee, because it shaped the final '
         'design twice. Slicing the history correctly is necessary but not '
         'sufficient: anything the system caches between decisions must also be fixed'
         ' by the decision date. A fitted model, a record of which model each product'
         ' is routed to, and a stored forecast are all derived from a particular '
         'history, and reusing one where a different history applies either '
         'reintroduces information the slicing removed or makes the result depend on '
         'what was computed before. The global model serving a date is therefore '
         'fixed by the date itself, on a refit grid described in Section 4.3, and the'
         ' model routing is recomputed whenever an earlier date is requested. Both '
         'rules are asserted by their own tests, and Section 5.9 reports the two '
         'defects that prompted them.')

    document.add_heading("3.13 Literature search strategy", level=2)
    para(document,
         "The approach set out in the AE1 progress report was as follows. Searches "
    ...
    log = pd.read_csv(DOCS / "search_log.csv", parse_dates=["date_bst"])
    first, last = log["date_bst"].min(), log["date_bst"].max()
    span = (f"{first.day} and {last.day} {last:%B %Y}" if (first.year, first.month) == (last.year, last.month)
            else f"{first.day} {first:%B %Y} and {last.day} {last:%B %Y}")
    para(document,
         "The claim in Section 2.10 that none of the sources reviewed studies replenishment in a "
         "small independent quick-service or casual-dining site is an assertion of absence, and an "
         "assertion of absence is only as good as the search behind it. The searches that can be "
         f"evidenced are those recorded while this document was drafted: {len(log)} web searches and "
         f"page fetches between {span}, grouped by purpose in Table 3.1.")
    rows = []
    for purpose, group in sorted(log.groupby("purpose"), key=lambda item: item[1]["date_bst"].min()):
        queries = group.loc[~group["query_or_url"].str.startswith("http"), "query_or_url"]
        start, end = group["date_bst"].min(), group["date_bst"].max()
        rows.append({
            "Purpose": purpose,
            "Searches": str(len(group)),
            "Dates": f"{start:%d %b}" if start.date() == end.date() else f"{start:%d %b} to {end:%d %b}",
            "Example": queries.iloc[0] if len(queries) else group["query_or_url"].iloc[0],
        })
    add_table(document, pd.DataFrame(rows),
              f"Table 3.1 - Recorded searches and page fetches, {span}, by purpose")
    para(document,
         'The log shows what the absence claim rests on, and what it does not. Only '
         'one recorded search was exploratory, a search on over-ordering as a cause '
         'of restaurant food waste, and it returned studies of customers over-'
         'ordering dishes rather than operators over-ordering stock. Every other '
         'search either retrieved UK sector statistics or located and checked a '
         'source that was already known. The four food-service studies discussed in '
         'Section 2.10 (Posch et al. 2022; Miguéis et al. 2022; Rodrigues et al. '
         '2024; Seyam et al. 2025) entered the draft through my own Chapter 2 text, '
         'and the recorded searches for them were made afterwards, to verify their '
         'details; the searches that first found them were not logged. The statement '
         'in Section 2.10 is therefore made about the sources reviewed, not as the '
         'result of a documented systematic search. A systematic search would record '
         'the databases, search strings, dates and the numbers of results screened '
         'and retained, and it remains to be done.')
...
def chapter_implementation(document: Document, ev: dict) -> None:
    ...
    add_figure(document, "fig_architecture.png",
               "Figure 4.1 - System architecture. Each box lists the modules present in that "
               "layer of the src/ tree, and the arrows show the direction in which data flows.")
    ...
    para(document,
         "A single service owns the operations that must not be duplicated: slicing history at the "
         "decision date, building leakage-free feature rows for the horizon, routing each SKU to its "
         "selected model, and holding the global model so it is refitted on a configured cadence "
         "rather than on every simulated day. Base-case forecasts are memoised by decision date and "
         "horizon, which is what makes sweeping the frontier affordable.")
    para(document,
         "The memo is valid only because the model behind a forecast is fixed by the forecast's date. "
         f"Refits fall every {ev['config']['forecast']['refit_frequency_days']} days on a grid anchored "
         "at the start of the simulation window and extended in both directions, and the model serving "
         "a decision date is the one fitted at the latest grid date on or before it. The model for a "
         "date therefore never depends on which dates were forecast earlier, and it never sees demand "
         "the decision could not. An earlier design refitted when the cached model became a given "
         "number of days old, which made the refit dates depend on the order in which dates were "
         "requested; Section 5.9 describes what that did.")
    para(document,
    ...
    para(document,
         ...
         "discovered.")
    para(document, "Figures 4.2 to 4.6 show the five pages for the decision date 5 December 2025, "
                   "captured from the running application.")
    for name, caption in [
        ('01_overview', 'Figure 4.2 - Overview page: products needing an order and their risk, with the replayed record of forecast against actual demand and its error.'),
        ('02_forecast', 'Figure 4.3 - Forecast page: history, the forecast with its prediction interval, and the drivers behind it.'),
        ('03_recommendations', 'Figure 4.4 - Recommendations page: the full order calculation for each product and the override facility.'),
        ('04_what_if', 'Figure 4.5 - What-if page: the recommendation recomputed under changed assumptions.'),
        ('05_simulation', 'Figure 4.6 - Simulation and evaluation page: the headline comparison, the waste and service-level frontier for both policies, and results over time and by product.'),
    ]:
        add_figure(document, f"screenshots/{name}.png", caption)
    ...
    para(document,
         f"The suite comprises {ev['test_count']} tests covering feature generation, forecast horizon "
         ...
         "the decision date moves backwards, forecasts that do not depend on the order in which dates "
         "are requested, the accuracy replay behind the overview panel, and "
...
def chapter_results(document: Document, ev: dict) -> None:
    ...
    document.add_heading("5.9 Four corrections arising during development", level=2)
    para(document,
         'Two defects were identified in the earlier baseline experiment and are '
         'reported here because they change how the earlier figures should be read. '
         'Two more were found later, in the delivered system, and are reported '
         'because the guarantees they threatened are the ones the operational results'
         ' rest on, and because correcting the second changed those results.')
    para(document,
    ...
    para(document,
         'The batch scripts were unaffected by this defect, because a backtest and a '
         'simulation advance through time and never request an earlier origin than '
         'the one they have reached; every figure then reported in this chapter was '
         'confirmed unchanged, byte for byte, after the correction. The exposure was '
         'confined to the dashboard, where the decision date is chosen freely and can'
         ' be revisited in any order. The correction discarded every cached artefact '
         'whenever the decision date moved backwards, and four tests asserted it, '
         'each verified to fail against the unfixed code. The fourth defect, below, '
         'later replaced the part of that rule concerning the fitted model with a '
         'stronger one.')
    para(document,
         ...
         "challenge it.")
    para(document,
         "The fourth defect was found while capturing the dashboard for this report. The Simulation "
         "page showed forecast-driven waste of 2,309.79 units, against 2,363.79 in the results file "
         "produced by the same code. The difference was reproduced exactly: opening the Overview page "
         "first left fourteen memoised forecasts behind, and the simulation then reused them. The memo "
         "was keyed by decision date and horizon, but each stored forecast had been made by whichever "
         "model the refit cadence happened to hold, and that cadence depended on the order in which "
         "dates had been requested. No model ever saw data from after its decision date, so this was "
         "not a leak, but the same simulation gave different results depending on what had been viewed "
         "before it.")
    para(document,
         "The correction fixes the refit dates to a grid, as described in Section 4.3, so that the memo "
         "is valid by construction. Three tests now assert it: one on the specific sequence that exposed "
         "it, a randomised check that forecasts are identical in any request order, and an end-to-end "
         "test that the simulation result does not depend on dashboard navigation. Each was confirmed to "
         "fail against the previous code, the last reproducing the 2,309.79 figure.")
    matched, matched_waste = ev["matched"], ev["matched_waste"]
    para(document,
         "Unlike the earlier corrections, this one changed the reported results, because the batch "
         "scripts had also been using the order-dependent cadence, only in a fixed order. At the two "
         "matched service levels the waste reductions moved from 25.8% and 38.4% to "
         f"{matched.iloc[0]['waste_reduction_pct']:.1f}% and {matched.iloc[1]['waste_reduction_pct']:.1f}%, "
         "the cost reductions from 10.4% and 13.9% to "
         f"{matched.iloc[0]['cost_reduction_pct']:.1f}% and {matched.iloc[1]['cost_reduction_pct']:.1f}%, "
         "and the service-level gains at matched waste from 1.15 and 1.62 percentage points to "
         f"{matched_waste.iloc[0]['service_level_gain_pp']:.2f} and "
         f"{matched_waste.iloc[1]['service_level_gain_pp']:.2f}. The direction and the conclusions are "
         "unchanged. The size of the movement, particularly in cost at the higher service level, is "
         "itself a finding: the operational results are sensitive to when the forecasting model is "
         "refitted, and a real deployment would need to fix and report its refit schedule as part of "
         "the method.")
    para(document,
...
def chapter_discussion(document: Document, ev: dict) -> None:
    ...
    para(document,
         ...
         "being real. That a restaurant adopting this system would see a "
         f"{ev['best_match']['waste_reduction_pct']:.1f}% reduction in "
    ...
    para(document,
         ...
         "demand forecasting in food catering services explicitly as a waste-reduction intervention. "
         "Tested on data from three canteens against baseline models built to mimic how the services "
         "estimate demand, their best forecasts would reduce wasted meals by 14% to 52% and unmet "
         "demand by 3% to 16%. This project's matched-availability waste reductions of "
         f"{ev['matched']['waste_reduction_pct'].min():.1f}% and "
         f"{ev['matched']['waste_reduction_pct'].max():.1f}% sit inside the first range, which is mildly "
         "reassuring in one narrow respect: it suggests the effect sizes embedded in this project's "
         "generator are not implausible for the sector. It should not be read as corroboration of the "
         "result. Their evaluation uses real operational data from three sites, whereas this one uses "
         "generated data for a single simulated site; a synthetic result agreeing with a real one is a "
         "statement about the generator's calibration, not about the system.")
    para(document,
    ...
    para(document,
         "Miguéis et al. (2022) are closer in framing, stating the waste-versus-availability "
    ...
    bullets(document, [
        ...
        "system-advised quantities, and Dietvorst, Simmons and Massey (2015) show that confidence in an "
...
def chapter_evaluation(document: Document, ev: dict) -> None:
    ...
    para(document,
         ...
         "of the UK GDPR and the Data Protection Act 2018 are met by "
         "design rather than by control; the Information Commissioner's Office guidance "
         "(Information Commissioner's Office n.d.) was used to confirm that position. Were "
    ...
    para(document,
         ...
         "system never places an order. Development followed the BCS Code of Conduct (BCS, The Chartered Institute for IT "
    ...
    para(document,
         'Materials for a usability evaluation with restaurant managers were prepared'
         ' on 25 August 2026: a participant information sheet, a consent form and a '
         '20-question questionnaire. The evaluation was not carried out, so no '
         'participant data was collected at any point and the project holds no '
         'personal data.')
...
def chapter_reflection(document: Document, ev: dict) -> None:
    ...
    for heading, paragraphs in REFLECTION:
        document.add_heading(heading, level=2)
        for text in paragraphs:
            para(document, text)
    page_break(document)
...
def cited_references(document: Document) -> list[str]:
    """Registry keys cited so far, in reference-list order. Stops the build on a bad citation."""
    findings, cited = citations.check(document_text(document))
    problems = [f for f in findings if f.kind != "UNCITED"]
    if problems:
        raise SystemExit("citation problems:\n" + "\n".join(
            f"  {f.kind} {f.cited!r} expected {f.expected!r}: ...{f.context[:90]}" for f in problems))
    return sorted(cited, key=references.sort_key)


def back_matter(document: Document, ev: dict) -> None:
    document.add_heading("References", level=1)
    for key in cited_references(document):
        paragraph = document.add_paragraph()
        for text, italic in references.render(key):
            run = paragraph.add_run(text)
            run.italic = italic
            run.font.size = Pt(10)
    page_break(document)
    ...
    for line in [
        ...
        "python scripts/capture_dashboard.py            # Section 4.7 screenshots, dashboard running",
        "python scripts/generate_dissertation_draft.py  # this document",
        f"python -m pytest tests/ -q                     # {ev['test_count']} tests",
    ]:
        paragraph = document.add_paragraph()
...
def main_text_words(document: Document) -> int:
    """Words in Chapters 1 to 9: no front matter, references, appendices or tables."""
    counting, words = False, 0
    for paragraph in document.paragraphs:
        if paragraph.style.name == "Heading 1":
            counting = paragraph.text == "1. Introduction" or (counting and paragraph.text != "References")
        if counting:
            words += len(paragraph.text.split())
    return words


def build(target: Path) -> Document:
    """Assemble the document, stamp its word count and date, and save it to ``target``."""
    ev = load_evidence()
    REPORT.mkdir(parents=True, exist_ok=True)
    subprocess.run([sys.executable, str(Path(__file__).with_name("architecture_diagram.py")),
                    str(PROJECT_ROOT), str(REPORT / "fig_architecture.png")], check=True)
    document = Document()
    ...
    stamp = next(p for p in document.paragraphs if p.text == WORD_COUNT_PREFIX)
    stamp.runs[0].text = f"{WORD_COUNT_PREFIX}{main_text_words(document):,}  |  Date: {today.day} {today:%B %Y}"
...
def main() -> int:
    ...
    document = build(TARGET)
    print(f"Wrote {TARGET}")
    print(f"  paragraphs : {len(document.paragraphs)}")
    print(f"  tables     : {len(document.tables)}")
    print(f"  main text  : {main_text_words(document):,} words (Chapters 1 to 9, excluding tables)")
    return 0
